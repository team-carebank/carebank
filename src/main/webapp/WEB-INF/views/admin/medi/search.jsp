...
<script>
	...
	$(document)
			.on(
					...
					function(e) {
						...
						let path = "${pageContext.request.contextPath}/admin/mtsearch.do?keyword=${keyword}&search=${searchType}&page=";
						if (span.id == 'prev') {
	...
	$(document).on("click", ".board-content-name", function(e) {
		...
		let path = "${pageContext.request.contextPath}/admin/mtinfo.do?id=";

		window.location.href = path + id;
	});

	$(document).on("click", "#add.board-content-config", function(e) {
		window.location.href = "mtwrite.do";
	})
</script>
<style>
		...
		<div class="body-content">
			...
			<div class="content-admin-main">
				<div class="admin-main-description">
					<h1>Search : Meditation</h1>
					<span>"${keyword }"에 대한 검색 결과를 표시합니다.</span>
				</div>
				<div class="admin-main-content">
					<div class="main-content-board">
						<div class="board-header">
							<div class="board-header-content">
								...
								<form method="post" action="mtsearch.do">
						...
						<div class="board-body">
							<c:forEach var="dto" items="${res }">
								<div class="board-body-content">
									<span id="supple_id">${dto.meditate_id }</span> <span
										class="board-content-name">${dto.med_title }</span> <span>${dto.subcat_name }</span>
									<span class="board-content-config" id="modify"
										onclick="window.location.href='mtmodi.do?id=${dto.meditate_id}'">수정하기</span>
								</div>
							</c:forEach>
							<c:if test="${empty res }">
								<div class="board-body-content">
					...
					<div class="main-content-search">
						<form method="post" action="mtsearch.do" id="search-keyword-data">
							<input type="hidden" name="page" value="1"> <input
